    ...
    : NetworkManager(nullptr)
{
    ...
    this->DicomReader = vtkSmartPointer<vtkDICOMImageReader>::New();
...
void MainWindow::on_actionLoadEnergy_triggered()
{
	...
	if (fileName != NULL)
	{
        ...
        this->EnergyReader->SetFileName(fileName.toStdString().c_str());
...
void MainWindow::on_actionLoadReference_triggered()
{
	...
	{
		// Read in the mhd file.
        this->ReferenceReader->SetFileName(fileName.toStdString().c_str());
...
void MainWindow::InitializeDicom()
{
    this->ui->energySpinBox->setEnabled(true);
    this->ui->xySlider->setEnabled(true);
    this->ui->xzSlider->setEnabled(true);
    ...
    this->EnergyMapper->SetInputConnection(DicomReader->GetOutputPort());
    this->EnergyMapper->SetBlendModeToComposite();

    //CT_Skin
    EnergyColor->AddRGBPoint( -3024, 0, 0, 0, 0.5, 0.0 );
    EnergyColor->AddRGBPoint( -1000, .62, .36, .18, 0.5, 0.0 );
    EnergyColor->AddRGBPoint( -500, .88, .60, .29, 0.33, 0.45 );
    EnergyColor->AddRGBPoint( 3071, .83, .66, 1, 0.5, 0.0 );

    EnergyOpacity->AddPoint(-3024, 0, 0.5, 0.0 );
    EnergyOpacity->AddPoint(-1000, 0, 0.5, 0.0 );
    EnergyOpacity->AddPoint(-500, 1.0, 0.33, 0.45 );
    EnergyOpacity->AddPoint(3071, 1.0, 0.5, 0.0);
    ...
    property->SetScalarOpacity(EnergyOpacity);
    property->SetColor(EnergyColor);
    property->ShadeOn();
    property->SetInterpolationTypeToLinear();

    EnergyVolume->SetProperty(property);
    EnergyVolume->SetMapper(EnergyMapper);
